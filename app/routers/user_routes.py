...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), session: AsyncSession = Depends(get_db)):
    settings = get_settings()  # Ensure settings are loaded correctly
    ...
    if user:
        ...
        # If authentication fails, raise an unauthorized error
...
async def update_professional_status(user_id: UUID, status_update: ProfessionalStatusUpdate, request: Request, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_role(["ADMIN", "MANAGER"]))):
    """
    ...
    """
    try:
        updated_user = await UserService.update_professional_status(db, user_id, status_update.dict(), current_user)
        if not updated_user:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
        return UserResponse.model_construct(
            id=updated_user.id,
        ...
        logger.exception("An unexpected error occurred while updating professional status")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An unexpected error occurred")
